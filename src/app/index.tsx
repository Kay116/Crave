...
export default function WelcomeScreen() {
  ...
  if (hasOnboarded) return <Redirect href="/preferences" />;
  const start = async () => { await completeOnboarding(); router.replace('/preferences'); };
  return <View style={styles.container}><Image source="https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?auto=format&fit=crop&w=1400&q=90" style={StyleSheet.absoluteFill} contentFit="cover" /><LinearGradient colors={['rgba(25,20,16,0.04)', 'rgba(25,20,16,0.2)', 'rgba(20,16,12,0.92)']} style={StyleSheet.absoluteFill} /><SafeAreaView style={styles.safe}><View style={styles.brand}><View style={styles.brandDot} /><Text style={styles.brandText}>CRAVE</Text></View><View style={styles.copy}><Text style={styles.eyebrow}>DINNER, DECIDED.</Text><Text style={styles.title}>Swipe until{`\n`}it feels right.</Text><Text style={styles.subtitle}>Skip the endless search. Follow your appetite, one beautiful plate at a time.</Text><Pressable onPress={start} style={({ pressed }) => [styles.button, pressed && styles.pressed]}><Text style={styles.buttonText}>Start craving</Text><Text style={styles.arrow}>→</Text></Pressable><Text style={styles.note}>No account needed</Text></View></SafeAreaView></View>;
}
